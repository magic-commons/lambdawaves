...
| W130 | two seams out of `boot()`, verbatim, one per commit: the keyboard dispatcher → `lab/keys.js` (5 edges; ACTIONS, toggleUI and the tab cycle stay), the camera law → `lab/camera-law.js` (6 edges; CAM exported); the phone block NOT cut — 20 boot bindings, 8 of them the preferences group, its phone/tablet state read from outside — so seams 12–17 wait for the preferences group to become one seam or for the layout seam. Landed after the public tag → ships in the next release | rack.js 5695 → 5447; lab/ net +41 (headers + imports ≈ half); serialize bytes identical on 11 snapshots; lock 208/208 per commit, 1004/1004 ×2 at the end; stylehash neutral 8 + 8 states; node 79/79; keyboard-window, input, render-regressions; on the merge: pwa + adopt in step, node gate green, lock GREEN ×2 (1004/1004), browser gate 120 GREEN with `current` red under a load of ~6 from other work (ResizeObserver loop messages in the DAW scene; then its valuesReveal race) and 14/14 solo; the same suite with 1.2 s waits passes on the merged and the pre-merge tree alike | seams (N1) |
...
| W128 | one controller: one median (the paced loop's presented intervals), one decision every ≥ 250 ms / 6 presents, one lever per decision — scale first, a grid rung only when a scale descent fails the scale² test or reaches the floor (with AUTO SCALE off: readers only), readers as before; the governor's ring/sort/judgement, the STEP ladder and the 3 s upward rung deleted | lab/ −13 lines; Electron 45 s: 128³ gas (table off) oscillating 64↔96 at scale 0.35 + steps 120, 96 fps → 64³ at scale 1, 240 steps, 2 grid changes, 115 fps; GLASS 128³: 0.35 + steps 120 + grid 64 → scale 0.55, grid 128; default 96³: 0 changes in 4/4; WIGNER parked as before; exports identical (3 scenes, one mid-play); `auto-scale.test` 23/23; lock GREEN ×2 | N0 dynamics |
