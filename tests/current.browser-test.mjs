...
try {
 ...
 const audioFace=await g.ev(`const M=__LW.mod.model,w=n=>new Promise(r=>setTimeout(r,n));M.deserialize(null);__LW.mod.addSource('audio');await w(250);const audio=M.sourceList().find(s=>s.kind==='audio');
  ...
  const ott={mixers:card.querySelectorAll('.aud-level-mix-dial').length,valuesHidden:rows.every(e=>getComputedStyle(e.querySelector('.aud-range-value')).opacity==='0'),
    ...
    aligned:rows.every((e,i)=>Math.abs((mixBoxes[i].y+mixBoxes[i].height/2)-(trackBoxes[i].y+trackBoxes[i].height/2))<2),
    ...
    routingBand:Math.abs(routeBoxes.low.height-44)<1&&Math.abs(routeBoxes.level.height-22)<1&&Math.abs(routeBoxes.hit.height-22)<1&&Math.abs(routeBoxes.mid.height-routeBoxes.low.height)<1&&Math.abs(routeBoxes.high.height-routeBoxes.low.height)<1,
  ...
  const result={allBands:compactRows.every(e=>getComputedStyle(e).display!=='none'),twoByTwo:new Set(knobBoxes.map(b=>Math.round(b.top))).size===2&&new Set(knobBoxes.map(b=>Math.round(b.left))).size===2,
    ...
    compactAligned:compactTracks.every((b,i)=>Math.abs((b.x+b.width/2)-(compactMix[i].x+compactMix[i].width/2))<1),labelsRight:compactNames.every((b,i)=>b.left>=compactTracks[i].right),shortLabels:compactRows.map(e=>e.querySelector('.aud-range-short').textContent).join('')==='MLMH',
    verticalHandles:compactRows.every(e=>e.querySelector('.aud-range-handle').getAttribute('aria-orientation')==='vertical'),routeWidth:Math.round(selectedRoute.width),latencyBeside:latencyBox.left>=selectedRoute.right&&latencyBox.top>=selectedRoute.top&&latencyBox.bottom<=selectedRoute.bottom,
    sourceButtonsNarrow:[...card2.querySelectorAll('.m2audsrc')].every(e=>e.getBoundingClientRect().width<=44),setVisible:[...card2.querySelectorAll('.m2audsrc')].some(e=>e.textContent.trim()==='SET'&&getComputedStyle(e).display!=='none'),
    ott,latency:card2.querySelector('.aud-latency').textContent,errors:__e.slice()};M.deserialize(null);return result;`);
 assert.equal(audioFace.allBands,true);assert.equal(audioFace.twoByTwo,true);assert.equal(audioFace.sideLayout,true);assert.equal(audioFace.controlsAboveKnobs,true);assert.equal(audioFace.verticalMeters,true);assert.equal(audioFace.compactAligned,true);assert.equal(audioFace.labelsRight,true);assert.equal(audioFace.shortLabels,true);assert.equal(audioFace.verticalHandles,true);assert.ok(audioFace.routeWidth<=88);assert.equal(audioFace.latencyBeside,true);assert.equal(audioFace.sourceButtonsNarrow,true);assert.equal(audioFace.setVisible,true);assert.equal(audioFace.latency,'— ms');assert.deepEqual(audioFace.errors,[]);
 for(const key of ['macroArc','equalRows','stacked','aligned','labelsAfter','shortLabels','longMeters','sourceNarrow','clearTrack','noPeakDots','palette','routingFour','routingBand','routingPlain','routeNamesNeutral','assignedNumber','noHeadStatus','valuesHidden','valuesReveal','lowMixHome','followsPalette','oneGrip'])assert.equal(audioFace.ott[key],true,key);
