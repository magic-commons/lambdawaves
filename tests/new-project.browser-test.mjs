...
try {
  ...
  const fresh = await g.ev(`const P = __LW.layout.projects, M = __LW.mod.model;
    ...
    const out = { ok, ms, asked, gum, status: __st(),
      ...
      nb: { open: __LW.layout.notebook.isOpen, text: __LW.notebook.text, title: __LW.notebook.title, subtitle: __LW.notebook.subtitle, stored: localStorage.getItem('lambdawaves.q0.notebook') },
  ...
  assert.deepEqual(fresh.nb, { open: false, text: '', title: 'NOTEBOOK', subtitle: '', stored: '' });
  ...
  /* ── 5 · THE NOTEBOOK LAW: a project opens onto its notebook only when the notebook has text ── */
  const law = await g.ev(`const P = __LW.layout.projects, data = JSON.parse(JSON.stringify(__LW.serialize()));
    ...
    const r = {};
    __LW.layout.notebook.open('notes'); r.blankFromOpen = P.open(blank) && !__LW.layout.notebook.isOpen;
    r.hello = P.open(hello) && __LW.layout.notebook.isOpen && __LW.layout.notebook.face === 'notes' && __LW.notebook.text === 'hello';
    r.blank = P.open(blank) && !__LW.layout.notebook.isOpen && P.current === 'probe/blank';
    r.errs = __e.slice(); return r;`);
  assert.deepEqual(law, { blankFromOpen: true, hello: true, blank: true, errs: [] });
  console.log('PASS the notebook law: a blank notebook (whitespace only) stays shut on open, even over an open notebook; text opens it onto its notes');
  ...
  /* ── 7 · ON A TABLET (an iPad UA: grid ceiling 96³) a saved 128³ × 240 project is clamped as it opens, and mat.steps follows the
