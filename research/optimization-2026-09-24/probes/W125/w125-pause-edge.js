/* probes/W125/w125-pause-edge.js — W125-2's gate, adapted from probes/A/governor.js (AUDIT-A FA4): the AXIAL GAS at 128³
...
const drag = async (ms = 3000) => {
  ...
  const r = { frames, presents: LW.stats.presents - pd, fps: +(frames / (ms / 1000)).toFixed(1), scaleChanges: changes, wentUp: ups, trace, end: { autoScale: LW.quality.autoScale, cw: canvas.width }, playing: LW.clock.playing, ema: +LW.autoQ.ema.toFixed(1) };
...
out.play = { secs: SECS, scaleTrace: drops, beforePause: { autoScale: LW.quality.autoScale, cw: canvas.width, res: F.resolution, drop: LW.governor.drop, ema: +LW.autoQ.ema.toFixed(1) } };
...
{ const tr = []; let up = 0, ch = 0, pv = LW.quality.autoScale, fr = 0; const a0 = performance.now(), pa = LW.stats.presents;
  ...
  out.spin = { frames: fr, presents: LW.stats.presents - pa, scaleChanges: ch, wentUp: up, trace: tr, end: { autoScale: LW.quality.autoScale, cw: canvas.width }, ema: +LW.autoQ.ema.toFixed(1), playing: LW.clock.playing };
  await LW.settle(); }
/* 4 · the stale window at 1 */
LW.quality.autoScale = 1; LW.play(); await sleep(5000);
const emaPlaying = +LW.autoQ.ema.toFixed(1); LW.quality.autoScale = 1; LW.pause(); for (let i = 0; i < 6; i++) await raf();
out.staleAtOne = { emaWhilePlaying: emaPlaying, emaAfterEdge: +LW.autoQ.ema.toFixed(1), drag: await drag() };
