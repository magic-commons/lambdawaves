/* probes/K/run.mjs — LANE K harness (a copy of probes/A/run.mjs that also reads a PREFILE): open the lab headless (Firefox, the gate's own driver), run ONE in-page probe body
 * (a file holding the body of an async function: it may await and must return JSON), write the result.
 *   LW_PORT=8721 GD_PORT=5231 node research/optimization-2026-09-24/probes/K/run.mjs <probe-body.js> [out.json] [query]
...
const body = (process.env.PREFILE ? fs.readFileSync(process.env.PREFILE, 'utf8') : (process.env.PRE || '')) + fs.readFileSync(process.argv[2], 'utf8');
