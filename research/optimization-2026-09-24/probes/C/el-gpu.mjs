...
const OUT = process.argv[2] || 'research/optimization-2026-09-24/probes/C/el-gpu.json';
...
try {
  ...
  await exp('blur radius 22 → 8 (diagnostic)', { cssB: 'html:root{--glass-blur:8px!important}' });
  await exp('frost OFF · disconnected: the 8 head color-matrix layers removed', { setupA: `__LW.setFrost('off')`, setupB: `__LW.setFrost('off')`, cssB: 'body.disconnected .dev>.dev-head' + NOBF });
  await ev(`__LW.setFrost('always'); await __LW.settle(); return 1;`);
  await exp('modulation window open vs closed', { setupA: `__LW.mod.collapse()`, setupB: `__LW.mod.expand()` });
  await exp('UI shown vs UI hidden', { setupA: `if(__LW.uiHidden) __LW.keys.toggleUI()`, setupB: `if(!__LW.uiHidden) __LW.keys.toggleUI()` });
