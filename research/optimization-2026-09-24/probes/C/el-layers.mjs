...
const OUT = process.argv[2] || 'research/optimization-2026-09-24/probes/C/el-layers.json';
...
try {
  ...
  await scene('default (disc · refractive · frost ALWAYS · light)', '');
  await scene('frost OFF (disc)', `__LW.setFrost('off')`);
  await scene('frost OFF · connected', `__LW.setDisconnected(false)`);
  await scene('frost ALWAYS · connected', `__LW.setFrost('always')`);
  await scene('frost ALWAYS · disc · modulation window open', `__LW.setDisconnected(true); __LW.mod.expand()`);
  await scene('UI hidden', `__LW.mod.collapse(); if(!__LW.uiHidden) __LW.keys.toggleUI()`);
  await scene('every window open', `if(__LW.uiHidden) __LW.keys.toggleUI(); for(const d of document.querySelectorAll('.dev')) d.classList.remove('closed','folded'); __LW.windowActivity.presentOffscreen(true)`);
