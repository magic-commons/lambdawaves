/* W128 copy of probes/PACE/pace-safari-emu.mjs: GOV=1 plays with the GOVERNOR on (the one controller: the grid must not move on this present-bound scene); the rest as the original.
...
try {
  ...
  await ev(`(async()=>{ __LW.pause(); __LW.governor.on=${process.env.GOV ? "true" : "false"}; __LW.quality.res=128; __LW.quality.steps=240; __LW.quality.scale=${process.env.SCALE || 2.5}; __LW.schedule(4); await __LW.settle(); await __LW.settle(); return 1; })()`);
  await sleep(600);
  ...
  const play = async (label, setup, ms = 5000) => ev(`(async()=>{ ${setup}; __LW.pause(); await __LW.settle(); await new Promise(r=>setTimeout(r,1500)); const P=window.__pace, S=__LW.stats, f=__LW.field, q=__LW.quality, A=__LW.autoQ;
    ...
    let n=0, depthMax=0, firstChange=null, maxDrop=0, minCap=Infinity; const pT=[], sc=[], depth=[]; let lp=S.presents;
    ...
    await new Promise(r=>{ const g=(ts)=>{ n++; const el=performance.now()-t0; const d=P.subs-P.done; depthMax=Math.max(depthMax, d); if ((n%30)===0) depth.push(d);
      maxDrop=Math.max(maxDrop, __LW.governor.drop); minCap=Math.min(minCap, f.stepCap); if (S.presents!==lp) { pT.push(ts); lp=S.presents; } if (firstChange===null && A.changes!==ch0) firstChange=Math.round(el);
    ...
    return { label:${JSON.stringify(label)}, seconds:+dt.toFixed(2), loopPerS:+(n/dt).toFixed(1), presentsPerS:+((S.presents-p0)/dt).toFixed(1), submitsPerS:+((P.subs-s0)/dt).toFixed(1),
      ...
      presentIntervalMedianMs:+(iv.length?iv[iv.length>>1]:0).toFixed(2), autoScaleFirstChangeMs:firstChange, autoScaleTrace:sc, canvas:[f.canvas.width,f.canvas.height], governor:__LW.governor.on, maxGridDrop:maxDrop, minStepCap:minCap===Infinity?'inf':minCap }; })()`);
