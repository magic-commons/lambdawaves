...
/* exactness of the replica and of the rg16 road, in rgba8 pixels */
const a8 = await readBack(rpipes.ship, BG4, W, H, V0), b8 = await readBack(rpipes.ship, BG2, W, H, V0);
out.replicaHash = fnv(a8.px, W, H, a8.bpr); out.replicaExact = out.replicaHash === rp.hash;
let pxDiff = 0, pxMax = 0; for (let i = 0; i < a8.px.length; i++) { const dd = Math.abs(a8.px[i] - b8.px[i]); if (dd) { pxDiff++; pxMax = Math.max(pxMax, dd); } }
out.rg16 = { hash: fnv(b8.px, W, H, b8.bpr), bytesDiffering: pxDiff, maxLevelDiff: pxMax };
/* marched samples per pixel */
{ const r = await readBack(rpipes.instr, BG4, W, H, V0, 'rgba32float'); const f = r.px, stride = r.bpr / 4; let hit = 0, it = 0, neg = 0, opaque = 0;
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) { const o = y * stride + x * 4; if (f[o + 2] > 0.5) { hit++; it += f[o]; neg += f[o + 1]; if (f[o + 3] > 0.985) opaque++; } }
  out.march = { pixels: W * H, hitBoxFraction: +(hit / (W * H)).toFixed(4), meanSamplesPerHitPixel: +(it / Math.max(1, hit)).toFixed(1), negligibleSampleFraction: +(neg / Math.max(1, it)).toFixed(4), earlyExitFraction: +(opaque / Math.max(1, hit)).toFixed(4), samplesPerFrameM: +(it / 1e6).toFixed(1) }; }
/* timings */
const TM = {};
TM.ship = await timeIt(rpipes.ship, BG4, T1, V0);
TM.rg16 = await timeIt(rpipes.ship, BG2, T1, V0);
TM.texFloor = await timeIt(rpipes.texFloor, BG4, T1, V0);
TM.texFloor_rg16 = await timeIt(rpipes.texFloor, BG2, T1, V0);
TM.dpr2 = await timeIt(rpipes.ship, BG4, T2, V0);
TM.dpr2_rg16 = await timeIt(rpipes.ship, BG2, T2, V0);
for (const s of [110, 160, 240]) TM['steps' + s] = await timeIt(rpipes.ship, BG4, T1, vWith({ 15: s }));
const VIEWS = ['density', 'phase', 'real', 'imag', 'diff', 'reim'], STY = ['cloud', 'solid', 'grain', 'signed', 'bands', 'dust', 'glass', 'additive'];
for (const v of [0, 1, 2, 5]) TM['view_' + VIEWS[v]] = await timeIt(rpipes.ship, BG4, T1, vWith({ 16: v, 28: 0, 39: 0 }));
for (const s of [0, 1, 3, 4, 5, 6, 7]) TM['style_' + STY[s]] = await timeIt(rpipes.ship, BG4, T1, vWith({ 28: s, 39: 0 }));
TM.finish_glass = await timeIt(rpipes.ship, BG4, T1, vWith({ 28: 0, 39: 1 }));
TM.palette_on_phase = await timeIt(rpipes.ship, BG4, T1, vWith({ 16: 1, 26: 1 }));
out.timing = TM;
out.note = 'min over ' + ROUNDS + ' rounds of n back-to-back renders into a ' + W + 'x' + H + ' rgba8unorm target (2x = ' + (2 * W) + 'x' + (2 * H) + '); tickErrMs = 100 ms / n';
