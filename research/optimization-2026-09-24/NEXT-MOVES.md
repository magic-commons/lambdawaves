...
|---|---|---|---|---|
| 127 | **Close the iPad: the project open** | the device report's "project open" scene splits the 1.3–1.5 s stall (desktop 69 ms); then remove from `restore()` whatever a closed card does not need synchronously (the layout rebuild, the modulation window's double rebuild, the theme repaint are the suspects) | the iPad scene before/after; `current.browser-test` round trip; serialize bytes; the lock | nothing (a bug-class fix) |
...
| 129 | **First-run hygiene** | bundled demos carry no look and no quality (WAVE DANCER still carries refractive, frost ALWAYS, camera friction 0); the newcomer's rack on a tablet/phone: SHADOW folded; MOLECULES + MO-REGISTRY off the rack, reachable from + and WINDOWS, with the chem-owner restore law like the H₂⁺ card's | official-defaults-palette; molecular-names retargeted; stylehash; the first-run node tests | SHADOW folded (his suggestion); MOLECULES off the rack (my yes, his call); a first-visit hint that Safari caps 120 Hz behind a flag |
