/* lab/keymap.js — visual keybinding editor and keyboard manual for λWAVES.
 *
 * ── WAVE 107 · THE GAME-MANUAL KEYBOARD AND THE REBINDING SEAM ──────────────────────────────────
 * A full-panel visual keyboard screen ported from the owner's desktop suite (Mandelbrot N-KEYBINDS)
 * into λWAVES.  A live picture of an ANSI keyboard drawn with every bound key lit and labelled, an
 * actions list beside it with two-way selection, and a conflict-guarding capture engine that lets
 * any shortcut be reassigned without touching storage directly or colliding by accident.
 *
 * ── THE SEVEN HOUSE LAWS EMBODIED HERE ──────────────────────────────────────────────────────────
 *
 * (1) ONE WIRE TO PERSISTENCE: `keys.bind(id, spec)` and `keys.reset()`.
 *     This module owns zero localStorage access and zero storage keys.  Persistence is the host
 *     model's responsibility; our promise is that every user act calls `keys.bind()` or `keys.reset()`
 *     and immediately calls `refresh()` to re-read the single source of truth (`keys.actions`).
 *
 * (2) NO LIVE REGIONS ANYWHERE (access.test.mjs A4 ceiling: 4).
 *     The application is capped at four live regions in the whole tree (#banner, .pj-status,
 *     .keys-say, .m2hint).  A fifth fails the accessibility gate.  Announcements are conveyed via
 *     ordinary readable text nodes (`.km-status`, `.km-hint`) and state attributes (`aria-pressed`,
 *     `aria-selected`).  No `role="status"`, `role="alert"`, or `aria-live` exists in this file.
 *
 * (3) SPACE BELONGS TO THE TRANSPORT ALONE (wave 88, Josh).
 *     Outside active recording, this module never intercepts Space, never binds Space to button
 *     activation, and never calls preventDefault on Space.  During RECORD INPUT, Space can of course
 *     be recorded as an action's new binding, matching the transport's own law.
 *
 * (4) THE TWO-PRESS STEAL CONFIRMATION.
 *     If the pressed key combination is already owned by another action, we DO NOT silently swap
 *     or steal it.  The first press displays the collision on the status line and names the action
 *     that currently holds it.  A second press of the identical combination confirms the steal,
 *     unbinding the prior holder and binding the selected action.  Escape cancels at any time.
 *
 * (5) MAC / IPAD · WINDOWS PARITY (A DISPLAY TOGGLE, NOT A SECOND TABLE).
 *     The segmented switch flips the modifier labels (⌘ / ⌥ on Mac vs Ctrl / Alt on Windows)
 *     across the keyboard's bottom row, the key cap modifier badges, and the list chips.  The
 *     bindings underneath are identical KeyboardEvent.code strings and boolean flags.
 *
 * (6) THE 44×44 CSS PX HIT TARGET LAW (access.test.mjs).
 *     Every interactive element is a real `<button type="button">`.  Smaller key caps expand their
 *     pointer hit area to at least 44×44 CSS px via a `::before` pseudo-element specified in the
 *     stylesheet contract below.
 *
 * (7) NO INLINE STYLES AND NO <style> TAGS.
 *     Every visual presentation is driven by class names prefixed with `km-`.  The exhaustive
 *     CSS contract is documented in the block comment at the conclusion of this file.
 */
