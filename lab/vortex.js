...
export function createVortex(host, overlay, api) {
  ...
  const jump = trig({ label: 'JUMP TO EVENT', title: 'scrub the clock to the next reconnection time (stretched three-mode states only)', onFire: () => { if (census && census.count && api.scrubTo) { const t = api.clock.t, Td = census.Td; const fires = [...new Set(census.points.filter((p) => p.admissible).map((p) => +p.t0.toFixed(6)))].sort((a, b) => a - b); let best = null; for (const f of fires) { let cand = f + Td * Math.floor((t - f) / Td + 1e-9) + Td; if (cand - t < 1e-6) cand += Td; if (best === null || cand < best) best = cand; } if (best !== null) api.scrubTo(best); } } }); r3.appendChild(jump.root);
  el('div', 'note', host).innerHTML = '<b>EXACT on the sampled circles.</b> Nodal lines are the unimodular roots of P(w) on every coaxial circle; a reconnection is a double root. Three stretched modes reconnect only at the roots of Φ(r) = Â₀² − 4|Â₊Â₋|, at two phases of the beat T_d = 2π/|E₊+E₋−2E₀| (Theorem C.2/C.3). Blind grids miss them — the algebra finds them.';
