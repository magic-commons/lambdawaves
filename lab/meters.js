...
export function createMeters(host) {
  ...
  function update(m) {
    ...
    energy.set(m.energy.toFixed(5)); energy.setSub((m.energy * HARTREE_EV).toFixed(3) + ' eV');
    if (m.t !== lastT) { tlog.set(m.t.toFixed(2), m.playing ? 'live' : ''); tlog.setSub((m.t * AU_TIME_AS).toFixed(1) + ' as'); lastT = m.t; }
