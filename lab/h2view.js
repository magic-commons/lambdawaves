...
export function createH2(host, api) {
  let on = false, which = 'triplet', R = 6, ke = 0.02, run = null, kappa = 300;   // κ: nuclear time per logical time unit (a DISPLAY choice)
  ...
  function refresh() {
    ...
    { const c = sto3gH2(R), wb = weinbaumOptimal(R, { iters: 24 }), lim = 2 * sto3gHydrogen().E;   // the correlated numbers AT the marker (24 golden steps: 3e-6 on ζ, 1e-11 on E — a readout, on a knob drag)
      ...
      roCI.setSub(`correlation ${(c.fci - c.rhf).toFixed(6)} · RHF − FCI ${(c.rhf - c.fci).toFixed(4)} (→ ∞ as R → ∞: a closed shell cannot dissociate) · Weinbaum ${wb.fci.toFixed(6)} at ζ = ${wb.zeta.toFixed(4)} · H + H on this basis = ${lim.toFixed(6)}`); }
    paint();
  }
  ...
  function paint() {
    ...
    const CAP = 'dot = EXACT H₂ (1.40 a₀, −1.1745) · HL is a bound from above · thin = STO-3G FCI, dashed = STO-3G RHF';
    ...
    g.fillStyle = T.ink(0.7); g.textAlign = 'right'; g.fillText('H + H', L - 3, y(-1));      // the gutter's one end value
